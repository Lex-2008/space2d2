...
function moveToNewStar(star, oldStar) {
    ...
    //2.5
    var a = commonNeighbour.neighbours.angleBetween(commonNeighbour.neighbours.right(newNeighbour), newNeighbour);
    ...
    commonNeighbour.link(newNeighbour, connectionToUse);
    // 2.5
    var a = commonNeighbour.neighbours.angleBetween(newNeighbour, commonNeighbour.neighbours.left(newNeighbour));
...
function check() {
    ...
    for (var c of player_star.neighbours) {
        if (Direction.normalize(c.value + 180) != c.target.neighbours.directionOf(player_star).value) {
            console.error('player_star and ... dont point to each other', c.target.name, Direction.normalize(c.value + 180), c.target.neighbours.directionOf(player_star).value);
            ok = false;
        }
        ...
        if (nextNeighbour.neighbours.directionOf(c.target).value != Direction.normalize(dirToNextNeighbour.value + 180)) {
            console.error('neighbours dont point to each other', c.target.name, nextNeighbour.name, nextNeighbour.neighbours.directionOf(c.target).value, Direction.normalize(dirToNextNeighbour.value + 180));
            ok = false;
        }
        if (measureAngle(c.target, c.target.neighbours.prev(nextNeighbour), nextNeighbour) +
            ...
            console.error('future common neighbour of two neighbours will have bad angle', c.target.name, nextNeighbour.name, 180 - measureAngle(c.target, c.target.neighbours.prev(nextNeighbour), nextNeighbour) -
